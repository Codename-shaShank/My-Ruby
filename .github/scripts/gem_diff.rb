...
def fetch_base_lockfile
  ...
  content = `git show origin/#{BASE_REF}:Gemfile.lock 2>/dev/null`
...
if changes.empty?
  begin
    ...
    File.write(File.join(repo_root, "gem_review.md"), "No gem version changes detected between this PR and #{BASE_REF}.")
    unless File.exist?(File.join(repo_root, "gem_review.md")) && !File.empty?(File.join(repo_root, "gem_review.md"))
    ...
    repo_root = `git rev-parse --show-toplevel`.strip
    File.write(File.join(repo_root, "gem_review.md"), "Error generating dependency review: #{e.message}")
    exit 1
  end
end
...
begin
  repo_root = `git rev-parse --show-toplevel`.strip
  File.write(File.join(repo_root, "gem_review.md"), markdown)
  unless File.exist?(File.join(repo_root, "gem_review.md")) && !File.empty?(File.join(repo_root, "gem_review.md"))
    warn "Failed to write gem_review.md"
    exit 1
  end
