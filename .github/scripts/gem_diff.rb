...
if changes.empty?
  begin
    File.write("gem_review.md", "No gem version changes detected between this PR and #{BASE_REF}.")
    unless File.exist?("gem_review.md") && !File.empty?("gem_review.md")
    ...
    File.write("gem_review.md", "Error generating dependency review: #{e.message}")
    exit 1
  end
end
...
begin
  File.write("gem_review.md", markdown)
  unless File.exist?("gem_review.md") && !File.empty?("gem_review.md")
    warn "Failed to write gem_review.md"
    exit 1
  end
